...
export function HeroBanner() {
  ...
  const [simplifyMotion, setSimplifyMotion] = useState(true)

  useEffect(() => {
    const mq = window.matchMedia('(max-width: 767px), (pointer: coarse)')
    const sync = () => setSimplifyMotion(mq.matches)
    sync()
    mq.addEventListener('change', sync)
    return () => mq.removeEventListener('change', sync)
  }, [])

  const softMotion = Boolean(reduceMotion || simplifyMotion)
  ...
  const imageY = useTransform(scrollYProgress, [0, 1], ['0%', softMotion ? '0%' : '18%'])
  const imageScale = useTransform(scrollYProgress, [0, 1], [1, softMotion ? 1 : 1.14])
  const contentOpacity = useTransform(scrollYProgress, [0, 0.4], [1, softMotion ? 1 : 0])
  const contentY = useTransform(scrollYProgress, [0, 0.4], [0, softMotion ? 0 : -48])
  ...
  const cardParallaxX = useTransform(springX, [-0.5, 0.5], softMotion ? [0, 0] : [10, -10])
  const cardParallaxY = useTransform(springY, [-0.5, 0.5], softMotion ? [0, 0] : [8, -8])
  const scrollCardY = useTransform(scrollYProgress, [0, 0.4], [0, softMotion ? 0 : 24])
  const cardY = useMotionTemplate`calc(${scrollCardY}px + ${cardParallaxY}px)`

  const rating = useCountUp(
    googleReviewsConfig.fallbackRating,
    cardReady,
  ...
  }, [mouseX, mouseY, softMotion])

  useEffect(() => {
    const id = window.setTimeout(() => setCardReady(true), softMotion ? 0 : 950)
    return () => window.clearTimeout(id)
  }, [softMotion])

  return (
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden bg-brand-deep">
      {/* Background — static on mobile to avoid GPU flicker from stacked transforms */}
      <motion.div
        style={
          softMotion
            ? undefined
        ...
        className="pointer-events-none absolute inset-0 md:inset-[-4%]"
      >
        <img
      ...
      <div className="pointer-events-none absolute inset-0 hidden opacity-[0.14] md:block grain" />
      {!softMotion ? (
        <div className="pointer-events-none absolute inset-0 hero-ambient" aria-hidden="true" />
      ) : null}

      <motion.div
        style={softMotion ? undefined : { opacity: contentOpacity, y: contentY }}
      ...
      >
        <div className="md:col-span-7 lg:col-span-6">
          <motion.p
            ...
            transition={{ duration: 0.7, delay: softMotion ? 0 : 0.28, ease }}
          ...
          <h1 className="mt-7 font-display text-[3.4rem] leading-[0.92] tracking-[-0.02em] text-paper sm:text-6xl md:mt-8 md:text-7xl lg:text-[5.5rem]">
            <span className="block overflow-hidden pb-1">
              <motion.span
                className="block"
                initial={softMotion ? false : { y: '108%' }}
                animate={{ y: 0 }}
                transition={{ duration: 0.95, delay: softMotion ? 0 : 0.4, ease }}
              >
                The Lavish
              </motion.span>
            </span>
            <span className="mt-0.5 block overflow-hidden pb-1">
              <motion.span
                ...
                initial={softMotion ? false : { y: '108%' }}
                animate={{ y: 0 }}
                transition={{ duration: 0.95, delay: softMotion ? 0 : 0.54, ease }}
              >
                Stay
              </motion.span>
            </span>
          ...
          <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.85, delay: softMotion ? 0 : 0.72, ease }}
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: softMotion ? 0 : 0.92, ease }}
        ...
        <motion.aside
          style={softMotion ? undefined : { x: cardParallaxX, y: cardY }}
          initial={{ opacity: 0, y: softMotion ? 12 : 0, x: softMotion ? 0 : 36 }}
          animate={{ opacity: 1, y: 0, x: 0 }}
          transition={{ duration: 0.95, delay: softMotion ? 0.1 : 1.05, ease }}
        ...
        >
          <div
            ref={cardRef}
            className={twMerge(
              'hero-glass-card group/card relative w-full max-w-sm overflow-hidden rounded-3xl border border-paper/12 bg-paper/[0.12] p-6 shadow-[0_28px_70px_rgba(20,23,20,0.28)] md:bg-paper/[0.09] md:p-7 md:backdrop-blur-2xl',
              !softMotion && 'hero-glass-shimmer',
            )}
          >
      ...
      <motion.a
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: softMotion ? 0.2 : 1.35, duration: 0.85, ease }}
      ...
      >
        ...
        <span className="hero-scroll-line relative block h-12 w-px overflow-hidden bg-paper/25">
          <motion.span
            ...
            animate={softMotion ? undefined : { y: ['-100%', '160%'] }}
        ...
        <motion.span
          ...
          animate={softMotion ? undefined : { y: [0, 3, 0], opacity: [0.55, 1, 0.55] }}
